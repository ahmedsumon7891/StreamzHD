...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if ("code" in body) patch.code = sanitizeText(body.code, 5).toUpperCase();
  ...
  const { data, error } = await supabaseAdmin.from("countries").update(patch).eq("id", id).select().single();
